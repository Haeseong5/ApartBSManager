...
class ApartAdapter (internal val context: Context,
                    internal val apartList: List<Apart>): BaseAdapter() {
...
//    init {
//        this.listBtnClickListener = onClickListener
//    }
    override fun getView(position: Int, convertView: View?, parent: ViewGroup?): View? {
        ...
        * */
        if (convertView == null){
            view = LayoutInflater.from(context).inflate(R.layout.item_apart, null)
            viewHolder = ViewHolder()
            viewHolder.tvName = view.findViewById(R.id.item_apart_name)
            viewHolder.btUpdate = view.findViewById(R.id.item_apart_update_button)
            viewHolder.btDelete = view.findViewById(R.id.item_apart_delete_button)
            view.tag = viewHolder
        }else{
        ...
        viewHolder.tvName?.text = apartList[position].name
        viewHolder.btUpdate?.tag = position
        viewHolder.btUpdate?.setOnClickListener{
            listBtnClickListener.onUpdateButtonClick(it, position)
        }
    ...
    override fun getItem(position: Int): Any {
        return apartList[position]
    }

    override fun getItemId(position: Int): Long {
        return apartList[position].id.toLong()
    }
